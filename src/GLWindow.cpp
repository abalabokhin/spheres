...
static const wchar_t GLWINDOW_CLASS_NAME[] = L"GLWindow_class";
...
//static double        g_timerFrequency = 1 / 1000.0;
...
bool GLWindowCreate(const wchar_t *title, int width, int height)
{
	ASSERT(title);
	ASSERT(width > 0);
	ASSERT(height > 0);
...
// основной цикл окна
int GLWindowMainLoop(std::vector<double> & timePerFrameList, GLubyte * pixels)
{
	MSG    msg;
	...
	while (g_window.running)
	{
		...
		if (g_window.running && g_window.active)
		{
			...
			//double beginFrameTime = GetTickCount();//GetTimerTicks();
			// рендер сцены
			...
			if (timePerFrameList.size() == 10) {
				if (pixels) {
					RECT    rect;
					GetClientRect(g_hWnd, &rect);
					glReadPixels(0, 0, rect.right - rect.left, rect.bottom - rect.top, GL_BGR, GL_UNSIGNED_BYTE, pixels);				
				}
				return 0;
			}
