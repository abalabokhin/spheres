...
GLuint ShaderProgramCreateFromMemory(std::string const & vertexShader, std::string const & fragmentShader)
{
	...
	sourceLength = vertexShader.length();
	shaderSource = new uint8_t[sourceLength];
	memcpy(shaderSource, vertexShader.c_str(), sourceLength);
	...
	sourceLength = fragmentShader.length();
	shaderSource = new uint8_t[sourceLength];
	memcpy(shaderSource, fragmentShader.c_str(), sourceLength);
	// зададим шейдеру исходный код и скомпилируем его
	glShaderSource(shader, 1, (const GLchar**)&shaderSource, (const GLint*)&sourceLength);
	glCompileShader(shader);
